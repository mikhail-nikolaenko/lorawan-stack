...
- The Gateway Server discards duplicate uplinks sent by faulty packet forwarder implementations. An incoming uplink message is considered a duplicate when it has the same PHYPayload, frequency and antenna index as the previous one. Duplicate uplink messages **are not** counted in the gateway connection statistics.
